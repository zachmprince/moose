...
x_pp = 'h'
y_pp = ['error_real', 'error_imag','error_azim_dB_dt_scalar']
...
for i in range(0, runs):
    ...
    # Read the file
    root = pyhit.load('scalar_azim_magnetic_time_deriv.i')
    ...
    pyhit.write("scalar_azim_magnetic_time_deriv.i", root)

    input_files = ['scalar_azim_magnetic_time_deriv.i']
...
fig.plot(data, label=['E_Real', 'E_Imag.','dB_dt_scalar'], marker='o', markersize=8)
fig.save('scalar_azim_magnetic_time_deriv_convergence.png')
data.to_csv('scalar_azim_magnetic_time_deriv_convergence.csv')
