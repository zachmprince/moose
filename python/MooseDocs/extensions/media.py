...
class MediaExtension(command.CommandExtension):
    ...
    def preRead(self, page):
        ...
        page["script_files"] = (
            set()
        )  # storage for image file created by scripts, see ScriptCommand object
    ...
    def preRender(self, page, ast, result):
        ...
        for extra in page["script_files"]:
            plot_page = pages.File(
                extra[0], source=extra[1], base=self.translator.destination
            )
    ...
    def latexImage(self, parent, token, page, src):
        ...
        img = latex.Command(
            env, "includegraphics", string=fname, args=args, escape=False
        )
        return img
...
class ScriptCommand(ImageCommand):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        result = subprocess.run(
            ["python", script_path], capture_output=True, text=True, env=run_env
        )
        ...
        if not os.path.isfile(plot_path):
            LOG.error(
                "The plot script '%s' must generate the plot '%s'",
                script_localname,
                plot_localname,
            )
...
class RenderImage(components.RenderComponent):
    ...
    def createLatex(self, parent, token, page):
        ...
        if src.startswith("http") and (ext not in (".jpg", ".png", ".pdf")):
            msg = (
                "Online images and images with the '{}' extension are not supported. The image "
            ...
            msg = (
                "Online images are not supported. The image should be downloaded. If the "
                "online version is desired for the website, the 'latex_src' setting can be used."
            )
            raise exceptions.MooseDocsException(msg, ext)
        elif ext not in (".jpg", ".png", ".pdf"):
            msg = (
                "Images with the '{}' extension are not supported. The image "
...
class RenderVideo(components.RenderComponent):
    ...
    def addVideoHelper(self, parent, token, page):
        ...
        if token["dark"]:
            html.Tag(
                video, "source", src=token["dark"], media="(prefers-color-scheme: dark)"
            )
    ...
    def createLatex(self, parent, token, page):
        ...
        if not src:
            msg = (
                "Videos ({}) are not supported with LaTeX output, the 'latex_src' setting "
        ...
        if ext not in (".jpg", ".png", ".pdf"):
            msg = (
                "Videos ({}) with the '{}' extension are not supported. The image "
                "should be converted to a '.jpg', '.png', or '.pdf'."
            )
