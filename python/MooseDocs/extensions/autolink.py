...
PAGE_LINK_RE = re.compile(
    r"(?P<filename>^(?!http).*?\.md)?(?P<bookmark>#.*)?", flags=re.UNICODE
)
LOG = logging.getLogger(__name__)

LocalLink = tokens.newToken("LocalLink", bookmark=None)
AutoLink = tokens.newToken(
    "AutoLink", page="", bookmark=None, alternative=None, optional=False, exact=False
)
...
class PageShortcutLinkComponent(core.ShortcutLinkInline):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        return token or core.ShortcutLinkInline.createToken(
...
class RenderLinkBase(components.RenderComponent):
    ...
    def createLatexHelper(self, parent, token, page, desired):
        ...
        if head is None:
            msg = (
                "The linked page ({}) does not contain a heading, so the filename "
...
class RenderAutoLink(RenderLinkBase):
    ...
    def createAutolinkHelper(self, parent, token, page):
        ...
        exact = token["exact"]
        try:
            desired = self.translator.findPage(
                token["page"],
                exact=exact,
    ...
    def createLatex(self, parent, token, page):
        ...
        desired = self.translator.findPage(
            token["page"], exact=token["exact"], throw_on_zero=throw
        )
        ...
        if desired is None and token["alternative"] is not None:
            msg = (
                "Warning: The 'alternative' setting for automatic links has no effect on LaTeX "
