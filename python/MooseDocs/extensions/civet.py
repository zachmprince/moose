...
CivetTestReport = tokens.newToken(
    "CivetTestReport", prefix=None, tests=list(), source=None
)
...
class CivetExtension(command.CommandExtension):
    ...
    def defaultConfig():
        ...
        config["remotes"] = (
            dict(),
            "Remote CIVET repositories to pull result; each item in "
            "the dict should have another dict with a 'url', 'repo', and 'repo_url'.",
        )
        config["branch"] = (
            "master",
            "The main stable branch for extracting test results.",
        )
        config["download_test_results"] = (
            True,
            "Automatically download and aggregate test results for the current merge commits.",
        )
        config["generate_test_reports"] = (
            True,
            "Generate test report pages, if results exist from download or local file(s).",
        )
        config["test_reports_location"] = (
            "civet",
    ...
    def init(self):
        ...
        if not self.__database and self.get("generate_test_reports", True):
            LOG.info(
                "CIVET test result reports are being disabled, it requires results to exist and the specified branch ('%s') to match the current repository.",
                self.get("branch"),
            )
        ...
        if self.get("generate_test_reports", True):
            ...
            for key, item in self.__database.items():
                ...
                result_pages.append(
                    pages.Source(
                        fullname, source=fullname, read=False, tokenize=False, key=key
                    )
                )
...
class CivetCommandBase(command.CommandComponent):
    ...
    def getCivetInfo(self, settings):
        ...
        if len(available) > 0:
            category = available.get(
                settings.get("remote") or list(available.keys())[0]
            )
