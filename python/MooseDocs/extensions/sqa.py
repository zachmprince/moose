...
SQARequirementMatrixHeading = tokens.newToken(
    "SQARequirementMatrixHeading", category=None
)
SQARequirementMatrixItem = tokens.newToken(
    "SQARequirementMatrixItem", label=None, reqname=None, satisfied=True
)
SQARequirementMatrixListItem = tokens.newToken(
    "SQARequirementMatrixListItem", label=None
)
SQARequirementText = tokens.newToken("SQARequirementText")
SQARequirementDesign = tokens.newToken(
    "SQARequirementDesign", design=[], line=None, filename=None
)
...
class SQAExtension(command.CommandExtension):
    ...
    def defaultConfig():
        ...
        config["repos"] = (
            dict(default="https://github.com/idaholab/moose"),
            "The repository locations for linking issues, set 'default' to allow "
            "'#1234' or add additional keys to allow for foo#1234.",
        )
        config["categories"] = (
            dict(),
            "A dictionary of category names that includes a "
            "dictionary with 'directories' and optionally 'specs' "
            ", 'dependencies', 'repo', 'reports'.",
        )
        config["include_non_testable_requirements"] = (
            False,
            "Control display of skipped requirements.",
        )
        config["requirement-groups"] = (
            dict(),
            "Allows requirement group names to be changed",
        )
        config["reports"] = (None, "Build SQA reports for dashboard creation.")
        config["default_collection"] = (
            "FUNCTIONAL",
            "The default requirement collection.",
        )
        config["append_run_exception_to_failure_analysis"] = (
            True,
            "Automatically include RunException tests in the 'FAILURE_ANALYSIS' collection.",
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if repo is not None:
            msg = (
                "The 'url' and 'repo' config options for MooseDocs.extensions.sqa are deprecated,"
    ...
    def preExecute(self):
        ...
        for app_report in itertools.chain.from_iterable(
            reports[2] for reports in self.__reports.values() if reports[2] is not None
        ):
            app_report.app_syntax = app_syntax
            app_report.exe_directory = exe_dir
            app_report.exe_name = (
                exe_name.rsplit("-", maxsplit=1)[0] if exe_name else None
            )
        ...
        for req_category in self.__requirements.values():
            for requirements in req_category.values():
                for req in requirements:
                    t_types = req.types
                    if (
                        (req.collections is None)
                        and (t_types is not None)
                        and ("RunException" in t_types)
                    ):
        ...
        LOG.info(
            "Gathering SQA requirement information complete [%s sec.]",
            time.time() - start,
        )
...
class SQARequirementsCommand(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        for group, requirements in self.extension.requirements(category).items():
            ...
            for req in requirements:
                ...
                if (
                    ...
                    and (req.collections is not None)
                    and any(c in collections for c in req.collections)
                ):
        ...
        if num_collection_reqs == 0:
            tokens.String(
                parent,
                content="No requirements of this type exist for this application, beyond those of its dependencies.",
            )

        return parent

    def _addRequirement(
        self, parent, info, page, req, requirements, category, settings
    ):
        collections = settings["collections"]
        ...
        if (
            (collections is not None)
            and (req.collections is not None)
            and not any(c in collections for c in req.collections)
        ):
            return
        if (types is not None) and not any(t in types for t in req.types or set()):
            return

        item = SQARequirementMatrixItem(
            parent, label=req.label, reqname=req.name, satisfied=req.testable
        )
        ...
        if settings.get("link", False):
            ...
            if settings.get("link-design", False) and req.design:
                SQARequirementDesign(
                    item, filename=req.filename, design=req.design, line=req.design_line
                )

            if settings.get("link-issues", False) and req.issues:
                SQARequirementIssues(
                    item,
                    filename=req.filename,
                    issues=req.issues,
                    line=req.issues_line,
                    url=self.extension.remote(category),
                )
            ...
            if settings.get("link-prerequisites", False) and req.prerequisites:
                ...
                for other in requirements:
                    if (other is not req) and (
                        req.prerequisites.intersection(other.names)
                    ):
            ...
            if (
                settings.get("link-results", False)
                and self.extension.hasCivetExtension()
            ):
...
class SQAVerificationCommand(SQARequirementsCommand):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        SQARequirementMatrixHeading(
            matrix, category=category, string=subcommand.title()
        )
        ...
        if num_vv_reqs == 0:
            tokens.String(
                parent,
                content="No test cases of this type exist for this application, beyond those of its dependencies.",
            )
...
class SQADependenciesCommand(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        if category == "moose":
            ...
            depends = (
                self.extension.dependencies(category)
                or self.extension.get("categories").keys()
            )
        ...
        for dep in depends:
            if dep != category:
                ...
                autolink.AutoLink(
                    core.ListItem(ul),
                    page="sqa/{}".format(fname),
                    optional=True,
                    warning=True,
                    class_="moose-sqa-dependency",
                )
        return parent
...
class SQADocumentCommand(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        return autolink.AutoLink(
            parent,
            page="sqa/{}_{}.md".format(category, suffix),
            optional=True,
            warning=True,
        )
...
class SQAReportCommand(command.CommandComponent):
    ...
    def _addRequirement(self, parent, token, page, req, requirements, category):
        ...
        text = SQARequirementText(item)
        if req.requirement is not None:
            self.translator.reader.tokenize(
                text,
                req.requirement,
                page,
                MarkdownReader.INLINE,
...
class RenderSQARequirementMatrixItem(core.RenderListItem):
    def createHTML(self, parent, token, page):
        ...
        num = html.Tag(
            li, "span", string=token["label"], class_="moose-sqa-requirement-number"
        )
    ...
    def createLatex(self, parent, token, page):
        ...
        if not token["satisfied"]:
            brace = latex.Brace()
            latex.Command(
                brace, "textcolor", args=[latex.Brace(string="red")], string=label
            )
            args = [brace]
        else:
...
class RenderSQARequirementIssues(components.RenderComponent):
    ...
    def __urlHelper(self, regex, name, issue, token):
        ...
        if match:
            ...
            if (key is not None) and (repo is None):
                ...
                raise exceptions.MooseDocsException(
                    msg, key, token["filename"], token["line"]
                )
    ...
    def getURL(self, issue, token):
        ...
        if (url is None) and (issue != ""):
            msg = (
                "Unknown issue number or commit (commit SHA-1 must be at least 10 digits): "
    ...
    def createLatex(self, parent, token, page):
        ...
        for issue in token["issues"]:
            ...
            if url is None:
                latex.Command(
                    parent, "textcolor", args=[latex.Brace(string="red")], string=issue
                )
            else:
                latex.Command(
...
class RenderSQAReport(components.RenderComponent):
    ...
    def createMaterialize(self, parent, token, page):
        ...
        for report in reports:
            ...
            html.Tag(
                hdr,
                "span",
                string=self._getBadgeText(report.status),
    ...
    def _addMessageModal(item, color, text):
        ...
        modal = html.Tag(
            item, "div", id_=str(unique_id), class_="modal modal-fixed-footer"
        )
