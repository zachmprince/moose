...
PaginationToken = tokens.newToken(
    "PaginationToken",
    previous=None,
    next=None,
    use_title=False,
...
class ContentExtension(command.CommandExtension):
    ...
    def defaultConfig():
        ...
        config["source_links"] = (
            dict(),
            "Dictionary of folder name to markdown links.",
        )
        return config
...
class ContentOutlineCommand(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        if settings["recursive"] and settings["pages"]:
            ...
            LOG.warning(
                common.report_error(
                    msg, page.source, info.line, info[0], prefix="WARNING"
                )
            )
...
class RenderContentToken(components.RenderComponent):
    ...
    def createHTML(self, parent, token, page):
        ...
        for head in sorted(headings.keys()):
            ...
            if head:
                h = html.Tag(
                    parent, "h{:d}".format(int(token["level"])), class_="moose-a-to-z"
                )
                if head in links:
