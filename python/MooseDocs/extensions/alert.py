...
AlertTitle = tokens.newToken(
    "AlertTitle", brand="", prefix=True, center=False, icon=True, icon_name=None
)
...
class AlertExtension(command.CommandExtension):
    ...
    def defaultConfig():
        ...
        config["use-title-prefix"] = (
            True,
            "Enable/disable including the brand (e.g., ERROR) as "
...
class RenderAlertToken(components.RenderComponent):
    ...
    def createHTML(self, parent, token, page):
        return html.Tag(
            parent, "div", class_="moose-alert moose-alert-{}".format(token["brand"])
        )
...
class RenderAlertTitle(components.RenderComponent):
    ...
    def createHTML(self, parent, token, page):
        ...
        if token.get("prefix"):
            brand = token["brand"]
            prefix = html.Tag(
                title, "span", string=brand, class_="moose-alert-title-brand"
            )
            if token.children:
