...
Code = tokens.newToken(
    "Code", content="", language="text", escape=True, max_height="350px"
)
...
class HeadingBlock(components.ReaderComponent):
    ...
    def createToken(self, parent, info, page, settings):
        heading = Heading(
            parent, level=info["level"].count("#"), **self.attributes(settings)
        )
        return heading
...
class UnorderedListBlock(ListBlock):
    ...
    ITEM_RE = re.compile(
        r"^- (?P<item>.*?)(?=\Z|^- )", flags=re.MULTILINE | re.DOTALL | re.UNICODE
    )
    TOKEN = "UnorderedList"
...
class LinkInline(components.ReaderComponent):
    ...
    RE = re.compile(
        r"\[(?!\S+?\][ .,])"  # start of link, see note above
...
class FormatInline(components.ReaderComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        if (tok == "^") or (tok == "@"):
            if not parent.children or (
                parent.children[-1].name not in ("Word", "Number")
            ):
                return None
...
class EscapeCharacter(components.ReaderComponent):
    RE = re.compile(
        r"\\(?P<char>\[|\]|!|\@|\^|\=|\*|\+|~|-)", flags=re.MULTILINE | re.DOTALL
    )
...
class RenderHeading(components.RenderComponent):
    ...
    def createLatex(self, parent, token, page):
        sec = latex.Command(
            parent, self.LATEX_SECTIONS[token["level"] - 1], start="\n", info=token.info
        )
        id_ = token.get("id")
        if id_:
...
class RenderCode(components.RenderComponent):
    def createReveal(self, parent, token, page):
        ...
        return self.createHTML(
            parent, token, page, style="font-size:65%", style_in_pre=False
        )
...
class RenderLink(components.RenderComponent):
    ...
    def createLatex(self, parent, token, page):
        ...
        if url.startswith("https://") or url.startswith("http://"):
            cmd = latex.Command(
                parent, "href", args=[latex.Brace(string=url)], info=token.info
            )
        else:
            cmd = latex.Command(
                parent,
...
class RenderString(components.RenderComponent):
    def createHTML(self, parent, token, page):
        return html.String(
            parent, content=token["content"], escape=token.get("escape", True)
        )
...
class RenderUnderline(components.RenderComponent):
    ...
    def createLatex(self, parent, token, page):
        for n in parent.path:
            if n.name in ["so", "ul"]:
                msg = (
                    "Nested strikethrough and underline commands are not supported in LaTeX, "
...
class RenderError(components.RenderComponent):
    ...
    def createMaterialize(self, parent, token, page):
        ...
        id_ = uuid.uuid4()
        a = html.Tag(
            parent, "a", class_="moose-exception modal-trigger", href="#{}".format(id_)
        )
        if token.info:
