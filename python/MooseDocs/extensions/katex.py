...
class KatexExtension(command.CommandExtension):
    ...
    def defaultConfig():
        ...
        config["prefix"] = (
            "Eq.",
            r"The prefix to used when referring to an equation by "
            r"the \\label content.",
        )
    ...
    def postTokenize(self, page, ast):
        ...
        if common.has_tokens(ast, "Equation") and isinstance(
            ...
            if self.get("macros", None):
                mc = ",".join(
                    '"{}":"{}"'.format(k, v) for k, v in self.get("macros").items()
                )
    ...
    def createToken(self, parent, info, page, settings):
        ...
        if inline and info["command"] == "equation":
            raise common.exceptions.MooseDocsException(
                "The '!equation' command is a block level command, use '!eq' instead."
            )
        if not inline and info["command"] == "eq":
            raise common.exceptions.MooseDocsException(
                "The '!eq' command is an inline level command, use '!equation' instead."
            )
        if inline and (settings.get("id", None) is not None):
            raise common.exceptions.MooseDocsException(
                "The 'id' setting is not allowed within in the inline equation command."
            )
...
class EquationReferenceCommand(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        if not inline:
            raise common.exceptions.MooseDocsException(
                "The '!eqref' command is an inline level command."
            )
...
class RenderEquation(components.RenderComponent):
    def createHTML(self, parent, token, page):
        ...
        content += 'katex.render("%s", element, {%s});' % (tex, config_str)
    ...
    def createLatex(self, parent, token, page):
        if token["inline"]:
            ...
            if token["label"]:
                latex.Command(
                    env, "label", string=token["label"], end="\n", escape=False
                )
