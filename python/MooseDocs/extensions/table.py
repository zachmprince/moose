...
class TableCommandComponent(command.CommandComponent):
    ...
    def createToken(self, parent, info, page, settings):
        ...
        flt = floats.create_float(
            parent, self.extension, self.reader, page, settings, token_type=TableFloat
        )
...
class TableComponent(components.ReaderComponent):
    ...
    FORMAT_RE = re.compile(
        r"^(?P<format>\|[ \|:\-]+\|)$", flags=re.MULTILINE | re.UNICODE
    )
    ...
    def createToken(self, parent, info, page, settings):
        ...
        if format_match:
            head = [
                item.strip()
                for item in content[: format_match.start("format") - 1].split("|")
                if item
            ]
            body = content[format_match.end("format") :]
            form = [
                item.strip() for item in format_match.group("format").split("|") if item
            ]
...
class RenderTable(components.RenderComponent):
    ...
    def createLatex(self, parent, token, page):
        ...
        args = [
            latex.Brace(string="\\textwidth", escape=False),
