...
AcronymListToken = tokens.newToken(
    "AcronymListToken", complete=False, location=None, heading=True
)
...
class AcronymExtension(command.CommandExtension):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__used = (
            set()
        )  # this is so that in-line definitions are rendered only once per page
...
class RenderAcronymToken(components.RenderComponent):
    ...
    def _createSpan(self, parent, token, page):
        ...
        if acro is None:
            tag = html.Tag(
                parent, "span", string=token["acronym"], class_="moose-error"
            )
            raise exceptions.MooseDocsException(
                "The acronym '{}' was not found.", token["acronym"]
            )
...
class RenderAcronymListToken(components.RenderComponent):
    ...
    def createLatex(self, parent, token, page):
        ...
        env = latex.Environment(
            ...
            "tabulary",
            args=[
                latex.Brace(None, string="\\linewidth", escape=False),
        ...
        if token["location"] is None:
            ...
            for node in self.translator.findPages(func):
                for key, value in self.extension.getAcronyms(node, False).items():
                    if key not in listed:
                        latex.String(
                            env, content="{}&{}\\\\".format(key, value), escape=False
                        )
                        listed.append(key)
