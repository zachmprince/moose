...
try:
    ...
    print(
        "The BeutifulSoup package (bs4) is required for the verify command",
...
def command_line_options(subparser, parent):
    ...
    parser = subparser.add_parser(
        "verify", parents=[parent], help="Testing only, do not use."
    )
    parser.add_argument(
        "-f",
        "--form",
...
def main(options):
    ...
    subprocess.check_output(
        cmd, cwd=os.path.join(MooseDocs.MOOSE_DIR, "python", "MooseDocs", "test")
    )
    ...
    for root, _, files in os.walk(out_dir):
        for fname in files:
            ...
            if ext == ".py":
                ...
                errno += compare(
                    os.path.join(root, fname), out_dir, gold_dir, options.update_gold
                )
