...
class TestTokenizeEquation(MooseDocsTestCase):
    ...
    def testBlockNoNumber(self):
        ...
        self.assertToken(
            ast(0), "Equation", tex="y=2x", inline=False, label=None, number=None
        )
    ...
    def testBlockWithNumber(self):
        ...
        self.assertToken(
            ast(0), "Equation", tex="y=2x", inline=False, label="foo", number=1
        )
        self.assertIn("moose-equation-", ast(0)["bookmark"])
    ...
    def testInline(self):
        ...
        self.assertToken(
            ast(0, 0), "Equation", tex="y=2x", inline=True, label=None, number=None
        )
...
class TestTokenizeEquationReference(MooseDocsTestCase):
    ...
    def testLocalEqRef(self):
        ...
        self.assertToken(ast(0), "Paragraph", size=1)
        self.assertToken(
            ast(0, 0), "EquationReference", size=0, label="second_law", filename=None
        )
...
class TestRenderEquation(MooseDocsTestCase):
    ...
    def testRenderBlockEquation(self):
        ...
        self.assertHTMLTag(
            res(0, 0), "span", size=0, class_="moose-katex-equation table-cell"
        )
        self.assertHTMLTag(
            res(0, 1),
            "script",
            size=1,
            string='var element = document.getElementById("None");katex.render("y=x", element, {displayMode:true,throwOnError:false});',
        )
...
class TestRenderEquationReference(MooseDocsTestCase):
    ...
    def testExceptions(self):
        ast = katex.EquationReference(
            None, label="first_law", filename="katex_include2.md"
        )
        with self.assertLogs(level=logging.ERROR) as cm:
            res = self.render(ast, renderer=base.HTMLRenderer())
        self.assertHTMLTag(res, "body", size=1)
