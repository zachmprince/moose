...
class TestPySyntax(unittest.TestCase):
    def testInfo(self):
        ...
        info = pysyntax.PySyntax.Info(
            "_TestObj__private_prop", TestObj._TestObj__private_prop
        )
        self.assertEqual(info.internal, False)
        ...
        info = pysyntax.PySyntax.Info(
            "_TestObj__privateMethod", TestObj._TestObj__privateMethod
        )
        self.assertEqual(info.internal, False)
        self.assertEqual(info.private, True)
        self.assertEqual(info.protected, False)
    ...
    def testClass(self):
        ...
        public = [
            ("publicMethod", Info("publicMethod", TestObj.publicMethod)),
            ("public_prop", Info("public_prop", TestObj.public_prop)),
        ]
        protected = [
            ("_protectedMethod", Info("_protectedMethod", TestObj._protectedMethod)),
            ("_protected_prop", Info("_protected_prop", TestObj._protected_prop)),
        ]
        private = [
            (
                "_TestObj__privateMethod",
                Info("_TestObj__privateMethod", TestObj._TestObj__privateMethod),
            ),
            (
                "_TestObj__private_prop",
                Info("_TestObj__private_prop", TestObj._TestObj__private_prop),
            ),
        ]
        internal = [
            (
                "__internalMethod__",
                Info("__internalMethod__", TestObj.__internalMethod__),
            ),
...
class TestPySyntaxExtension(MooseDocsTestCase):
    ...
    def testClassCommandAST(self):
        ast = self.tokenize(
            "!pysyntax class name=MooseDocs.extensions.pysyntax.PySyntaxExtension"
        )
        ...
        self.assertToken(
            ...
            "Heading",
            level=2,
            class_="moose-pysyntax-class-heading",
            string="MooseDocs.extensions.pysyntax.PySyntaxExtension",
        )
        self.assertToken(
            ast(0, 1),
            "Monospace",
    ...
    def testFunctionCommandAST(self):
        ...
        self.assertToken(
            ast(0, 0), "Heading", level=2, class_="moose-pysyntax-member-heading"
        )
