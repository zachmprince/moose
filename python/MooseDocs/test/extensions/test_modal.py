...
class TestModalLink(MooseDocsTestCase):
    ...
    def testHTML(self):
        ...
        self.assertHTMLTag(
            res(0), "span", size=1, class_="moose-modal-link", string="test"
        )
    ...
    def testMaterialize(self):
        ...
        # Content as Token
        ast = modal.ModalLink(
            None, string="test", content=core.Code(None, content="1+2=3")
        )
        ...
        self.assertHTMLTag(
            res(1, 0, 0), "pre", size=1, class_="", style="max-height:350px;"
        )
        self.assertHTMLTag(
            res(1, 0, 0, 0),
            "code",
...
class TestModalSourceLink(MooseDocsTestCase):
    ...
    def testHTML(self):
        ...
        self.assertHTMLTag(
            ...
            "span",
            size=1,
            class_="moose-source-filename",
            string="(python/MooseDocs/test/extensions/test)",
        )
        ...
        self.assertHTMLTag(res, "body", size=1)
        self.assertHTMLTag(
            res(0), "span", size=1, class_="moose-source-filename", string="string"
        )
        ...
        self.assertHTMLTag(
            ...
            size=1,
            class_="moose-source-filename",
            string="(Foo: python/MooseDocs/test/extensions/test)",
        )
    ...
    def testMaterialize(self):
        ...
        self.assertHTMLTag(
            res(1, 0, 1), "pre", size=1, class_="", style="max-height:350px;"
        )
        self.assertHTMLTag(
            res(1, 0, 1, 0), "code", size=1, class_="language-text", style=""
        )

        self.assertIn(
            "MOOSE Application Standard Makefile", res(1, 0, 1, 0, 0)["content"]
        )
        self.assertHTMLTag(res(1, 1), "div", size=1, class_="modal-footer")
        self.assertHTMLTag(
            res(1, 1, 0), "a", size=1, class_="modal-close btn-flat", string="Close"
        )
        ...
        self.assertHTMLTag(
            res(0),
            "a",
            string="(Foo: python/MooseDocs/test/extensions/framework/Makefile)",
        )
...
class TestModalSourceLinkDisableSource(MooseDocsTestCase):
    ...
    def testMaterialize(self):
        ...
        ast = modal.ModalSourceLink(
            None, src="moose/test/tests/kernels/simple_diffusion/simple_diffusion.i"
        )
        res = self.render(ast, renderer=base.MaterializeRenderer())
        self.assertSize(res, 2)
        self.assertHTMLTag(
    ...
    def testLatex(self):
        ...
        self.assertLatexString(
            res(0), content="(python/MooseDocs/test/extensions/framework/Makefile)"
        )
