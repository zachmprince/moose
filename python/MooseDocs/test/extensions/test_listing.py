...
def extractContent(filename, opts=dict()):
    ...
    content, _ = common.extractContent(
        common.read(MOOSE_DIR + "/" + filename), settings
    )
    return content
...
class TestListingNumbers(MooseDocsTestCase):
    ...
    def testAST(self):
        ...
        for i, s in enumerate(self.SHORTCUTS):
            ...
            self.assertToken(
                ast(i, 0), "FloatCaption", key=s, prefix="Listing", number=i + 1
            )
            self.assertToken(
                ast(i, 1),
                "ListingCode",
                content=s.capitalize(),
    ...
    def testHTML(self):
        ...
        for i, s in enumerate(self.SHORTCUTS):
            ...
            self._assertHTML(
                res(i, 0), res(i, 1), res(3, i), "Listing " + str(i + 1), s
            )
    ...
    def testMaterialize(self):
        ...
        for i, s in enumerate(self.SHORTCUTS):
            ...
            self._assertHTML(
                res(i, 0, 0), res(i, 0, 1), res(3, i), "Listing " + str(i + 1), s
            )
    ...
    def testLatex(self):
        ...
        for i, j, s in zip([0, 2, 4], [1, 3, 5], self.SHORTCUTS):
            ...
            self.assertLatexString(
                res(i)["args"][0](2), content="\\mbox{}", escape=False
            )
...
class TestListingCaptions(MooseDocsTestCase):
    ...
    def testAST(self):
        ...
        self.assertToken(
            ast(0), "Code", content=self.CODE[0], max_height=MAX_HEIGHT, language=None
        )
        ...
        self.assertToken(ast(0), "Listing", size=2)
        self.assertToken(
            ast(0, 0), "FloatCaption", size=3, key="file1", prefix="File", number=1
        )
        self.assertToken(ast(0, 0, 0), "Word", content="the")
        self.assertToken(ast(0, 0, 1), "Space", count=1)
        ...
        self.assertToken(
            ast(2), "Shortcut", size=1, key="file1", link="#file1", string="File 1"
        )
    ...
    def testMaterialize(self):
        ...
        self.assertHTMLTag(
            res(0, 0, 0, 0), "span", size=1, class_="moose-caption-heading"
        )
...
class TestListingWithSpace(MooseDocsTestCase):
    ...
    def testAST(self):
        ...
        self.assertSize(ast, 1)
        self.assertToken(
            ast(0), "Code", content=self.CODE, max_height="92px", language=None
        )
...
class TestListingLanguage(MooseDocsTestCase):
    ...
    def testAST(self):
        ...
        self.assertSize(ast, 1)
        self.assertToken(
            ast(0), "Code", content=self.CODE, max_height=MAX_HEIGHT, language="c++"
        )
...
class TestFileListing(MooseDocsTestCase):
    ...
    CODE.append(
        extractContent(FILE, opts=dict(start="InputParameters", end="::Diffusion"))
    )
    TEXT.append(
        "!listing " + FILE + " start=InputParameters end=::Diffusion max-height=92px"
    )
    ...
    def testAST(self):
        ...
        self.assertToken(ast(0), "Listing", size=2)
        self.assertToken(
            ast(0, 0), "FloatCaption", key="diffusion-c", prefix="Listing", number=1
        )
        self.assertToken(
            ast(0, 1),
            "ListingCode",
        ...
        self.assertSize(ast, 2)
        self.assertToken(
            ast(0), "Code", content=self.CODE[1], max_height="92px", language="cpp"
        )
    ...
    def testMaterialize(self):
        ...
        self._assertHTML(res(0), "max-height:92px;", self.CODE[1])
        self.assertHTMLTag(
            res(1), "a", size=1, class_="moose-source-filename tooltipped modal-trigger"
        )
        self.assertHTMLString(res(1, 0), content="({})".format(self.FILE))
...
class TestFileListingDiff(MooseDocsTestCase):
    ...
    AFTER_PREFIX = "Bar:"
    PREFIXED_TEST = (
        f"{TEXT} before_link_prefix={BEFORE_PREFIX} after_link_prefix={AFTER_PREFIX}"
    )
    ...
    def testHTML(self):
        ...
        self.assertHTMLTag(
            ...
            "code",
            size=1,
            class_="language-diff-cpp diff-highlight",
            style="",
        )
        self.assertHTMLString(res(0, 0)(0), content=self.DIFF)
        self.assertHTMLTag(res(1), "span", size=1, class_="moose-source-filename")
        self.assertHTMLString(
            res(1, 0),
    ...
    def testMaterialize(self):
        ...
        self.assertHTMLString(res(2), "<br>")
        self.assertHTMLTag(
            res(3), "a", size=1, class_="moose-source-filename tooltipped modal-trigger"
        )
        self.assertHTMLString(
            res(3, 0),
            content=f"({listing.FileListingCommand.DEFAULT_AFTER_LINK_PREFIX} {self.FILE})",
...
class TestInputListing(MooseDocsTestCase):
    ...
    CODE = [
        extractContent(FILE[0], opts=dict(start="Mesh", end="Variables"))
        + "\n"
    ...
    CODE.append(
        extractContent(FILE[3], opts={"end": "exodiff", "include-end": True})
        + "\n"
    ...
    def testAST(self):
        ...
        self.assertToken(
            ast(0, 0), "FloatCaption", key="prfx", prefix="xxxxx", number=1
        )
        ...
        self.assertToken(
            ast(1), "Shortcut", size=1, key="prfx", link="#prfx", string="Xxxxx 1"
        )
