...
class TestContentList(MooseDocsTestCase):
    ...
    def testMaterialize(self):
        _, res = self.execute(
            "!content list location=extensions", renderer=base.MaterializeRenderer()
        )
    ...
    def _assertHTML(self, res):
        ...
        self.assertHTMLTag(col(0)(0), "li", size=1)
        self.assertHTMLTag(
            col(0)(0)(0), "a", href="extensions/core.html", string="Core Extension"
        )
    ...
    def testLatex(self):
        _, res = self.execute(
            "!content list location=extensions", renderer=base.LatexRenderer()
        )
...
class TestContentAtoZ(MooseDocsTestCase):
    ...
    def setupContent(self):
        config = [
            dict(
                ...
                content=[
                    "extensions/core.md",
                    "extensions/content.md",
                    "extensions/acronym.md",
                    "extensions/table.md",
                ],
            )
        ]
    ...
    def testAST(self):
        ...
        self.assertSize(ast, 1)
        self.assertToken(
            ast(0), "AtoZToken", size=0, level=2, location="", buttons=True
        )

        ast = self.tokenize("!content a-to-z location=foo level=42 buttons=False")
        self.assertSize(ast, 1)
        self.assertToken(
            ast(0), "AtoZToken", size=0, level=42, location="foo", buttons=False
        )
    ...
    def testMaterialize(self):
        _, res = self.execute(
            "!content a-to-z location=extensions", renderer=base.MaterializeRenderer()
        )
        ...
        for i in range(0, len(b)):
            if b[i] in ["A", "C", "T"]:
                self.assertHTMLTag(
                    res(0)(i), "a", class_="btn moose-a-to-z-button", string=b[i]
                )
            else:
                self.assertHTMLTag(
                    res(0)(i),
    ...
    def testLatex(self):
        _, res = self.execute(
            "!content a-to-z location=extensions", renderer=base.LatexRenderer()
        )
    ...
    def _assertHTML(self, res):
        ...
        self.assertHTMLTag(col(0)(0), "li", size=1)
        self.assertHTMLTag(
            col(0)(0)(0), "a", href="extensions/table.html", string="Table Extensions"
        )
...
class TestContentOutline(MooseDocsTestCase):
    ...
    HEADINGS.append(
        {
            ...
            "Multiline Quotation": {
                "page": "core",
                "id": "multiline-quotation",
                "level": 3,
            },
            "Nested Quotations and Lists": {
                "page": "core",
                "id": "nested-quotations-and-lists",
                "level": 3,
            },
            "Headings": {"page": "core", "id": "headings", "level": 2},
            "One": {"page": "core", "id": "one", "level": 1},
            ...
            "Single level lists": {
                "page": "core",
                "id": "unordered-single-level-lists",
                "level": 3,
            },
            "Nested lists": {
                "page": "core",
                "id": "unordered-nested-lists",
                "level": 3,
            },
            "Ordered List": {"page": "core", "id": "ordered-list", "level": 2},
            "`Single level lists": {
                "page": "core",
                "id": "ordered-single-level-lists",
                "level": 3,
            },
    ...
    def testMaterialize(self):
        for case in range(len(self.TEXT)):
            res = self.render(
                self.tokenize(self.TEXT[case]), renderer=base.MaterializeRenderer()
            )
...
class testContentPagination(MooseDocsTestCase):
    ...
    MARGINS = [
        ["10.2%", "-2pt"],
        ["24px", "24px"],
    ]  # second test uses default '24px' margins
    TEXT = (
    ...
    def testHTML(self):
        ...
        self.assertHTMLTag(
            res(0)(0), "a", size=1, class_="moose-content-previous", href=prev
        )
        self.assertHTMLString(res(0)(0)(0), content="Previous")
        self.assertHTMLTag(
            res(0)(1), "a", size=1, class_="moose-content-next", href=next
        )
        ...
        self.assertHTMLTag(
            res(1)(0), "a", size=1, class_="moose-content-previous", href=prev
        )
        self.assertHTMLString(res(1)(0)(0), content="Material Icon")
        self.assertHTMLTag(
            res(1)(1), "a", size=1, class_="moose-content-next", href=next
        )
    ...
    def testMaterialize(self):
        ...
        self.assertHTMLTag(
            res(0)(0), "a", size=2, class_="moose-content-previous btn", href=prev
        )
        self.assertHTMLString(res(0)(0)(0), content="Previous")
        self.assertHTMLTag(res(0)(0)(1), "i", size=1, class_="material-icons left")
        self.assertHTMLString(res(0)(0)(1)(0), content="arrow_back")
        self.assertHTMLTag(
            res(0)(1), "a", size=2, class_="moose-content-next btn", href=next
        )
        self.assertHTMLString(res(0)(1)(0), content="Next")
        ...
        self.assertHTMLTag(
            res(1)(0), "a", size=2, class_="moose-content-previous btn", href=prev
        )
        self.assertHTMLString(res(1)(0)(0), content="Material Icon")
        self.assertHTMLTag(res(1)(0)(1), "i", size=1, class_="material-icons left")
        self.assertHTMLString(res(1)(0)(1)(0), content="arrow_back")
        self.assertHTMLTag(
            res(1)(1), "a", size=2, class_="moose-content-next btn", href=next
        )
        self.assertHTMLString(res(1)(1)(0), content="Config Extension")
...
class TestMissingExternalContentList(MooseDocsTestCase):
    ...
    def setupContent(self):
        config = [
            dict(
                root_dir="python/MooseDocs/test/invalid_content",
                content=["file.md"],
                external=True,
            )
        ]
        ...
        self.assertIn(
            "Part of the content specified in 'config.yml' is not included in documentation",
            cm.output[0],
        )
