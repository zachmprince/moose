...
class TestSettings(MooseDocsTestCase):
    ...
    def testNoFloatAST(self):
        ast = self.tokenize(
            "!devel settings module=MooseDocs.extensions.devel object=SettingsCommand"
        )
    ...
    def testFloatAST(self):
        ast = self.tokenize(
            "!devel settings id=foo module=MooseDocs.extensions.devel object=SettingsCommand"
        )
    ...
    def testErrors(self):
        ...
        self.assertToken(
            ast(0), "ErrorToken", message="The 'module' setting is required."
        )

        ast = self.tokenize("!devel settings module=MooseDocs.extensions.devel")
        self.assertToken(
            ast(0), "ErrorToken", message="The 'object' setting is required."
        )

        ast = self.tokenize("!devel settings module=wrong object=SettingsCommand")
        self.assertToken(
            ast(0), "ErrorToken", message="Unable to load the 'wrong' module."
        )

        ast = self.tokenize(
            "!devel settings module=MooseDocs.extensions.devel object=wrong"
        )
        self.assertToken(
            ast(0),
            "ErrorToken",
            message="Unable to load the 'wrong' attribute from the 'MooseDocs.extensions.devel' module.",
        )
