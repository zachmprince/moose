...
class TestShortcutLink(MooseDocsTestCase):
    ...
    def testAutoLink(self):
        ...
        self.assertToken(
            ...
            page="core.md",
            bookmark=None,
            alternative="content.md#bookmark",
            optional=False,
            exact=False,
        )
        ...
        ast = self.tokenize(
            "[core.md#bookmark alternative=content.md optional=True exact=True]"
        )
        self.assertSize(ast, 1)
        self.assertToken(ast(0), "Paragraph", size=1)
        self.assertToken(
...
class TestLink(MooseDocsTestCase):
    ...
    def testAutoLink(self):
        ...
        ast = self.tokenize(
            "[core](core.md#bookmark alternative=content.md optional=True exact=True)"
        )
        self.assertSize(ast, 1)
        self.assertToken(ast(0), "Paragraph", size=1)
        self.assertToken(
...
class TestAutoLinkRender(MooseDocsTestCase):
    ...
    def setupContent(self):
        config = [
            dict(
                root_dir="python/MooseDocs/test/content", content=["extensions/core.md"]
            )
        ]
    ...
    def _testBookmarkHelper(self, link):
        ...
        self.assertHTMLTag(res, "body", size=1)
        self.assertHTMLTag(
            res(0), "a", size=3, href="extensions/core.html#unordered-nested-lists"
        )
    ...
    def testAlternative(self):
        ...
        self.assertLatexString(
            res(0),
            "Warning: The 'alternative' setting for automatic links "
    ...
    def testExactError(self, mock_log):
        ...
        # TODO: Use assertLogs in python 3.4
        self.assertIn(
            "Unable to locate a page that ends with the name 'core.md'",
            mock_log.call_args[0][0],
        )

    def testExactAlternative(self):
        link = autolink.AutoLink(
            None, page="not_a_file.md", alternative="extensions/core.md", exact=True
        )
    ...
    def testOptionalAlternative(self):
        link = autolink.AutoLink(
            None, page="not_a_file.md", alternative="not_a_file.md", optional=True
        )
    ...
    def testBookmark(self):
        link = autolink.AutoLink(
            None, page="core.md", bookmark="unordered-nested-lists"
        )
