#!/usr/bin/env python3
"""Tests moosepy.tree.search.iterate."""
import unittest
...
def build_tree():
    ...
    moosetree.Node(B, "BB", year=1949)
    moosetree.Node(B, "BC")
    moosetree.Node(B, "BD")

    moosetree.Node(C, "CA")
    moosetree.Node(C, "CB", year=1980)
    CC = moosetree.Node(C, "CC")

    moosetree.Node(D, "DA")
    moosetree.Node(D, "DB")

    moosetree.Node(AB, "ABA")
    moosetree.Node(AB, "ABB")
    ABC = moosetree.Node(AB, "ABC", year=1980)
    moosetree.Node(AB, "ABD")

    moosetree.Node(BA, "BAA")
    moosetree.Node(BA, "BAB", year=1954)

    moosetree.Node(CC, "CCA")
    moosetree.Node(CC, "CCB")
    moosetree.Node(CC, "CCC")

    ABCA = moosetree.Node(ABC, "ABCA")
    moosetree.Node(ABC, "ABCB")

    moosetree.Node(ABCA, "ABCAA")
    moosetree.Node(ABCA, "ABCAB", year=2013)
    return root
...
class TestIterator(unittest.TestCase):
    ...
    def testPreOrder(self):
        """Test moosepy.tree.search.iterate with pre-order method."""
        root = build_tree()
    ...
    def testBreadthFirst(self):
        """Test moosepy.tree.search.iterate with breadth-first method."""
        root = build_tree()
